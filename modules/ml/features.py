...
class Features:
    ...
    def _get_global_rank(self):
        try:
            return int(re.findall(r"Global Rank: ([0-9]+)", self.rank_checker_response.text)[0])
        except:
            return -1
...
def _check_ssl_state(url):
    response = _get_url_response(url)
    return int(response.text)


def _check_domain_reg_len(url):
